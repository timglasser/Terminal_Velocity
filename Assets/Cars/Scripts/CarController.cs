...
namespace UnityStandardAssets.Vehicles.Car
{
  ...
  //      private float trigTime = 1f;
        ...
        {
//			Debug.Log("Move control drive " + gameObject.name);
            for (int i = 0; i < 4; i++)
            {
                Quaternion quat;
                Vector3 position;
                m_WheelColliders[i].GetWorldPose(out position, out quat);
//				Debug.Log("Car control drive " + gameObject.name );
//				Debug.Log("Wheel control drive " +  m_WheelMeshes[i].name  + "wpos = " + position);
        ...
        public bool AnySkidSoundPlaying()
        {
            for (int i = 0; i < 4; i++)
            {
                if (m_WheelEffects[i].PlayingAudio)
