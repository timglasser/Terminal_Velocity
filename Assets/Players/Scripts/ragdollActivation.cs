...
public class ragdollActivation : MonoBehaviour {
	...
	public bool ragdolled
	{
		...
		set{
			if (value==true){
			...
			else { // set ragdoll to false block
				if (state==RagdollState.ragdolled) {
					...
					if (anim.GetBoneTransform(HumanBodyBones.Hips).forward.y>0) //hip forward vector pointing upwards, initiate the get up from back animation
					{
