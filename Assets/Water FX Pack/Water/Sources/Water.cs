...
public class Water : MonoBehaviour
{
	...
	{
		if(  !gameObject.GetComponent<Renderer>().sharedMaterial || !gameObject.GetComponent<Renderer>().enabled )
			return;
		...
		if( mode >= WaterMode.Reflective )
		{
			...
			gameObject.GetComponent<Renderer>().sharedMaterial.SetTexture( "_ReflectionTex", m_ReflectionTexture );
		}
		...
		if( mode >= WaterMode.Refractive )
		{
			...
			gameObject.GetComponent<Renderer>().sharedMaterial.SetTexture( "_RefractionTex", m_RefractionTexture );
		}
	...
	/*	Vector4 waveSpeed = mat.GetVector( "WaveSpeed" );
	   ...
	   Vector4 waveScale4 = new Vector4(waveScale, waveScale, waveScale * 0.4f, waveScale * 0.45f);
		...
		Vector3 waterSize = gameObject.GetComponent<Renderer>().bounds.size;		
	...
	private void CreateWaterObjects( Camera currentCamera, out Camera reflectionCamera, out Camera refractionCamera )
	{
		...
		if( mode >= WaterMode.Reflective )
		{
			...
			if( !reflectionCamera ) // catch both not-in-dictionary and in-dictionary-but-deleted-GO
			{
				...
				reflectionCamera = Camera.current;
	...
	{
		if( !SystemInfo.supportsRenderTextures || !gameObject.GetComponent<Renderer>() )
